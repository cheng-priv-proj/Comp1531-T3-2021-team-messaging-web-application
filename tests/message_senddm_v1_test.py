...
def register():
    ...
    user_id_dict = requests.post(url + 'auth/register/v2', data = {
    ...
    dm_id_dict = requests.post(url + 'dm/create/v2', data = {
...
def test_senddm_one_valid_message(clear, register, extract_token, extract_user, extract_dm, extract_message):
    ...
    now = datetime.now()
    message_id = extract_message(requests.post(url + 'message/senddm/v1', data = {
        'token': owner_token,
        'dm_id': dm_id,
        'message': 'testmessage' }).json())
    messages = requests.get(url + 'message/senddm/v2', params = {
        'token': owner_token,
        'dm_id': dm_id, 
        'start': 0 }).json()
...
def test_senddm_multiple_valid_messages(clear, register, extract_token, extract_user, extract_dm, extract_message):
    ...
    now = datetime.now()
    message_id0 = extract_message(requests.post(url + 'message/senddm/v2', data = {
        'token': owner_token,
        'dm_id': dm_id,
        'message': 'testmessage' }).json())
    message_id1 = extract_message(requests.post(url + 'message/senddm/v2', data = {
        'token': owner_token,
        'dm_id': dm_id,
        'message': 'testmessage0' }).json())
    message_id2 = extract_message(requests.post(url + 'message/senddm/v2', data = {
        'token': owner_token,
        'dm_id': dm_id,
...
def test_senddm_invalid_message_to_short(clear, register, extract_token, extract_dm):
    ...
    assert requests.post(url + 'messages/senddm/v1', data = {
        'token': owner_token,
        'dm_id': dm_id,
        'message': ''
    }).status_code == 400
