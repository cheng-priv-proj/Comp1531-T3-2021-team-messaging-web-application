...
def dm_create_v1(auth_id, u_ids):
    ...
    user_info = data_store.get_user_from_u_id_dict()

    # Obtaining the handle_strs for all members in channel
    u_ids.append(auth_id)
    handle_str_list = [user_info.get(u_id).get('handle_str') for u_id in u_ids]

    # Checking whether there were any invalid u_ids
    if none in handle_str_list:
    ...
    dm_name = ', '.join(handle_str_list.sort())
    dm_id = (len(data_store.get_dms_from_dm_id_dict()) + 1) * -1

    data_store.insert_dm(auth_id, dm_id, u_ids, dm_name)
